-- retrieves reseauencers' states, number of messages in states 
SELECT
  ...
  gs.status as "GrpStatus", m.status as "MsgStatus", count(1) as "Count",
  SUBSTR(gs.component_dn,-INSTR(reverse(gs.component_dn),'/') + 1) as "Component",
  gs.container_id as "ContainerId"
FROM
  mediator_group_status gs, mediator_resequencer_message m
WHERE
